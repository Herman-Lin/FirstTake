...
<?php
      ...
      function setNote( $Note ) {
        global $connection;
        $query = "INSERT INTO notes (Judge, Video, Note) VALUES (\"{$_GET['usr']}\",8,\"{$Note}\")";
        $result = mysqli_query($connection , $query);
      }

      function getAllNotes() {
        global $connection;
        $query = "SELECT Note FROM notes WHERE Video = 8 && Judge = \"{$_GET['usr']}\"";
        $result = mysqli_query($connection, $query);
      ...
      if ($_POST['formName'] != "score") {
        ...
        header("Location: /entry_9.php?usr={$_GET['usr']}");
      ...
      <div id="container">
        <div id="header">
          ...
          <h1 align=right><br><br>Priscila Alegria Nunez - "Heart Her"</h1>
        </div>
      </div>
