...
<?php
      ...
      function setNote( $Note ) {
        global $connection;
        $query = "INSERT INTO notes (Judge, Video, Note) VALUES (\"{$_GET['usr']}\",12,\"{$Note}\")";
        $result = mysqli_query($connection , $query);
      }

      function getAllNotes() {
        global $connection;
        $query = "SELECT Note FROM notes WHERE Video = 12 && Judge = \"{$_GET['usr']}\"";
        $result = mysqli_query($connection, $query);
      ...
      if ($_POST['formName'] != "score") {
        ...
        header("Location: /entry_D.php?usr={$_GET['usr']}");
      ...
      <div id="container">
        <div id="header">
          ...
          <h1 align=right><br><br>Alexander Knox - "Follow Your True North"</h1>
        </div>
      </div>
